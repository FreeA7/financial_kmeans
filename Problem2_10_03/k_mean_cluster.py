...
# Obtain the return rates in each cluster
        ...
        for i in range(0, len(k_means.labels_)):
            if k_means.labels_[i] == 1:
                cluster_1_return_rate.append(returni[3 + i])
            else:
                cluster_0_return_rate.append(returni[3 + i])

        mean_return_rate = np.mean(returni[3:-18])
        mean_cluster_0_return_rate = np.mean(cluster_0_return_rate)
        mean_cluster_1_return_rate = np.mean(cluster_1_return_rate)

        std_return_rate = np.std(returni[3:-18])
        std_cluster_0_return_rate = np.std(cluster_0_return_rate)
        std_cluster_1_return_rate = np.std(cluster_1_return_rate)

        print('***** Cluster Finished *****')
        print('Average daily return and volatility (overall)：' + str(mean_return_rate) + '\t' + str(std_return_rate))
